...
def make_3d_plot(output, params, fparams, file_spec=""):
    ...
    values = [
        ...
        output.T_save,
        output.k_save,
        output.perturbation[:, :: params.save_dt_sim],
    ]
    title = ["U [m/s]", "V [m/s]", r"$\theta$ [K]", r"TKE [$m^2/s^2$]", "perturbation"]
    file_names = [
        "u_over_time_z_" + file_spec + ".png",
    ...
    for index in range(len(title)):
        ...
        plt.rc("font", **{"size": "11"})
        plt.pcolor(
            X, Y, values[index], cmap=colours[index]
        )  # , vmin=color_range[index][0], vmax=color_range[index][1])
        plt.xlabel("time [h]")
        plt.ylabel("z [m]")
        plt.colorbar()
        plt.title(title[index])
        plt.savefig(
            str(output.solution_directory) + file_names[index], bbox_inches="tight"
        )
