...
def find_steady_state_fixed_height(data_u, data_v, data_delta_theta, data_tke):
    ...
    data_u["rol_mean"] = data_u["sim_0"].rolling(num_hours * one_h_num_steps, min_periods=1).mean()
    data_v["rol_mean"] = data_v["sim_0"].rolling(num_hours * one_h_num_steps, min_periods=1).mean()
    data_delta_theta["rol_mean"] = data_delta_theta["sim_0"].rolling(num_hours * one_h_num_steps, min_periods=1).mean()
    data_tke["rol_mean"] = data_tke["sim_0"].rolling(num_hours * one_h_num_steps, min_periods=1).mean()
    ...
    for k, v in data_tke.groupby((data_tke["bel_thresh"].shift() != data_tke["bel_thresh"]).cumsum()):
        if v["bel_thresh"].all():
            if len(v) >= len_steady_state_h * one_h_num_steps:
    ...
    try:
        steady_state = reduce(np.intersect1d,
            (steady_state_range_u, steady_state_range_v, steady_state_range_delta_theta, steady_state_range_tke),
        )[0]
    except Exception:
        pass
...
def save_initial_cond_for_single_variable_to_file(curr_steady_state, data_file_path, init_file_path, variable_name):
...
def save_initial_cond_for_all_variable_to_file(stab_func_type, uG_value, steady_state, data_path):
    ...
    for var in variables:
        save_initial_cond_for_single_variable_to_file(steady_state, data_path, init_file_path, var)


def plot_steady_state_for_all_variables(vis_path, uG, data_u, data_v, data_delta_theta, data_tke, steady_state):
    ...
    ax[0].plot(data_u.iloc[time_range[0]: time_range[1], 1], data_u.iloc[time_range[0]: time_range[1], 0],
               color='black')
    ax[1].plot(data_delta_theta.iloc[time_range[0]: time_range[1], 1],
               data_delta_theta.iloc[time_range[0]: time_range[1], 0], color='black')
    ax[2].plot(data_v.iloc[time_range[0]: time_range[1], 1], data_v.iloc[time_range[0]: time_range[1], 0],
               color='black')
    ax[3].plot(data_tke.iloc[time_range[0]: time_range[1], 1], data_tke.iloc[time_range[0]: time_range[1], 0],
               color='black')

    ax[0].axvspan(data_u.iloc[steady_state, 1], data_u.iloc[60 + steady_state, 1], color="red", alpha=0.5,
                  label="steady state region")
    ax[1].axvspan(data_delta_theta.iloc[steady_state, 1], data_delta_theta.iloc[60 + steady_state, 1], color="red", alpha=0.5,
                  label="steady state region")
    ax[2].axvspan(data_v.iloc[steady_state, 1], data_v.iloc[60 + steady_state, 1], color="red", alpha=0.5,
                  label="steady state region")
    ax[3].axvspan(data_tke.iloc[steady_state, 1], data_tke.iloc[60 + steady_state, 1], color="red", alpha=0.5,
                  label="steady state region")
...
for uG in uG_range:
    ...
    try:
        ...
        df_u, df_v, df_delta_theta, df_tke, _ = prepare_data.create_df_for_fixed_z(data_path, curr_file,
                                                                                   steady_state_height)
    except Exception:
    ...
    # Plot steady state
    plot_steady_state_for_all_variables(vis_path, uG, df_u, df_v, df_delta_theta, df_tke, steady_state)
