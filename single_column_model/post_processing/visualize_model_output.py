...
def make_3D_plot(data_path, vis_path, file_name, curr_param, variable_name, suffix='', z_max=''):
...
def plot_delta_theta_over_u(vis_path, data_u, data_delta_theta, suffix):
    ...
    cmap = matplotlib.cm.get_cmap('cmc.batlow', len(data_delta_theta.columns[:-1])).colors
    ...
    plt.savefig(vis_path + '/delta_theta_over_u' + suffix + '.png', bbox_inches='tight', dpi=300)
...
def plot_histogram(vis_path, data, variable_name, suffix):
    ...
    plt.savefig(vis_path + '/histogram_of_' + variable_name + suffix + '.png', bbox_inches='tight', dpi=300)
...
def plot_1D_stoch_process(directory_path, vis_path, file_path):
    ...
    plt.savefig(vis_path + '/stoch_process_' + str(file_path) + '.png', bbox_inches='tight', dpi=300)
...
def plot_2D_stoch_process(directory_path, vis_path, file_path):
    ...
    plt.savefig(vis_path + '/perturbation_' + str(file_path) + '.png', bbox_inches='tight', dpi=300)
...
def plot_transitioned_solutions(data_path, vis_path, list_files, var, data_delta_theta, z_top):
    data_delta_theta = data_delta_theta.drop(columns=['time'])

    # Find all columns where all values are non nan
    data_delta_theta_subset_non_nan = data_delta_theta[data_delta_theta.columns[~data_delta_theta.isna().any()]]
    ...
    for column in data_delta_theta_subset_non_nan.columns:
        col_subset = data_delta_theta_subset_non_nan[column].rolling(2).agg(_check_values_increasing)
    ...
    for column in data_delta_theta_subset.columns:
        ...
        make_3D_plot(data_path, vis_path, file_name, var, 'v', suffix=column)
        make_3D_plot(data_path, vis_path, file_name, var, 'theta', suffix=column)
