...
for idx, file in enumerate(output_files_long_tail):
    ...
    ax.scatter(
        wind_speed, delta_theta, color="black", s=10
    )  # , label='long-tail' if idx == 0 else "", marker='s')
...
# Sort by wind speed
_, mean_long_tail_wind, mean_long_tail_delta_theta = zip(
    *sorted(zip(uG_list, mean_long_tail_wind, mean_long_tail_delta_theta))
)

ax.plot(
    mean_long_tail_wind, mean_long_tail_delta_theta, color="black", label="long-tail"
)
...
for idx, file in enumerate(output_files_short_tail):
    wind_speed, delta_theta, _, _ = get_data(file)
    ax.scatter(
        wind_speed, delta_theta, color="blue", s=10
    )  # , label='short-tail' if idx == 0 else "")
    uG_list.append(float(file.split("_")[-5]))
    mean_short_tail_delta_theta.append(np.mean(delta_theta))
    mean_short_tail_wind.append(np.mean(wind_speed))

# Sort by wind speed
_, mean_short_tail_wind, mean_short_tail_delta_theta = zip(
    *sorted(zip(uG_list, mean_short_tail_wind, mean_short_tail_delta_theta))
)
...
plt.savefig(
    f"{output_directory}stab_func_bifurcation_plot_{perturbation_strength}.png",
    bbox_inches="tight",
    dpi=300,
)
