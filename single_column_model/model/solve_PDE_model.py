...
def add_perturbation_to_weak_form_of_model(
    ...
    if (
        perturbation_param == "u"
        or perturbation_param == "theta"
        or perturbation_param == "u and v"
    ):
...
def get_richardson_number_stochastic_grid(fenics_param, model_param):
    ...
    richardson_num_det = transform_values.project_fenics_function_to_numpy_array(
        richardson_num_det, fenics_param.Q
    )
    # Interpolate richardson number from deterministic to stochastic grid
    interpolation_func = interpolate.interp1d(
        fenics_param.z[:, 0], richardson_num_det, fill_value="extrapolate"
    )
...
def solution_loop(params, output, fenics_params, stoch_solver, u_n, v_n, theta_n, k_n):
    ...
    for time_idx in range(params.num_steps):
        if params.perturbation_param != "stab_func":
            ...
            # Solve PDE model with Finite Element Method in space
            solver = define_PDE_model.prepare_fenics_solver(
                fenics_params, fenics_params.F
            )
        ...
        if params.perturbation_param == "stab_func":
            ...
            for _ in range(tau_s):
                stoch_solver.evolve(
                    phi_0, dtau_s, Lambda, Upsilon, Sigma, sqrt_dtau_s, params.lz
                )
            ...
            phi_sol_extrapolated = interpolate.interp1d(
                params.stoch_grid[:, 0], phi_sol_stoch_grid, fill_value="extrapolate"
            )
            phi_sol_det_grid[0 : params.Hs_det_idx] = phi_sol_extrapolated(
                fenics_params.z[0 : params.Hs_det_idx, 0]
            )
        ...
        v_sol_np = transform_values.interpolate_fenics_function_to_numpy_array(
            v_sol, fenics_params.Q
        )

        # Get corresponding heat flux
        Kh_sol_np = transform_values.project_fenics_function_to_numpy_array(
            define_PDE_model.K_h(fenics_params, params), fenics_params.Q
        )
